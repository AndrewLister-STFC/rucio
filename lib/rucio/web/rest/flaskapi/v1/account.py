...
class AccountLimits(MethodView):
    ...
    def get(self, account, rse=None):
        ...
        try:
            if rse:
                limits = get_account_limit(account=account, rse=rse, vo=request.environ.get('vo'))
            else:
                limits = get_account_limits(account=account, vo=request.environ.get('vo'))
...
class Usage1(MethodView):
    ...
    def get(self, account):
        ...
        try:
            data = ""
            for usage in get_account_usage(account=account, rse=None, issuer=request.environ.get('issuer'), vo=request.environ.get('vo')):
...
class Usage2(MethodView):
    ...
    def get(self, account, rse):
        ...
        try:
            data = ""
            for usage in get_account_usage(account=account, rse=rse, issuer=request.environ.get('issuer'), vo=request.environ.get('vo')):
                data += dumps(usage, cls=APIEncoder) + '\n'
            return Response(data, content_type="application/x-json-stream")
        except AccountNotFound as error:
            return generate_http_error_flask(404, 'AccountNotFound', error.args[0])
